...
)
{
    ...
    if ( Work == NULL ) work = (double *) malloc (4*n*sizeof (double)) ;
    else                work = Work ;
    if ( work == NULL )
    {
    ...
    for (iter = 1; iter <= maxit; iter++)
    {
        ...
        if ( IterQuad == Parm->qrestart ) QuadF = TRUE ;
        IterRestart++ ;
        if ( !Com.AWolfe )
        {
...
)
{
    ...
    double alpha, a, a1, a2, b, bmin, B, da, db, d0, d1, d2, dB, df, f, fa, fb, fB,
...
)
{
   ...
   /* Level 0 = no printing, ... , Level 3 = maximum printing */
    ...
    /* for a QuadStep, function evaluated on interval [psi_lo, phi_hi]*psi2*previous step */
    Parm->psi_lo = 0.1 ;
    Parm->psi_hi = 10. ;
