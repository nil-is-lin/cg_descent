/* After the first iteration, the line search might take too large a
   step, when it tries to bracket a local minimizer along the search
   direction. In this expansion phase, the expansion factor is rho.
   The default value of rho is 5. Thus, in each step of the expansion
   phase, the bracketing interval grows by a factor of 5. If the cost
   function contains a log or an exponential function, this
   expansion might result in either an overflow or an attempt to compute
   the log of a negative number. A less aggressive expansion may avoid
   this problem. In the following example, we choose a small initial
   stepsize (Step is set to 1, and initial stepsize is 1.e-5), QuadStep is
   turned off (set to 0), and rho is 1.5. The code has to do a number
   of expansions to reach a suitable interval bracketing the minimizer
   in the initial search direction.

   Termination status: 0
   Convergence tolerance for gradient satisfied
   absolute largest component of gradient: 6.642185e-09
   ...
   We then set rho = 5. In this case, fewer expansions are needed to bracket
   the minimizer in the initial search direction:

   Termination status: 0
   Convergence tolerance for gradient satisfied
   absolute largest component of gradient: 6.147910e-09
   function value: -6.530787e+02
   cg iterations: 32
...
1.5       rho          (interval growth factor used to get bracketing interval)
