...
)
{
    double  *d, *g, *xtemp, *gtemp, *d1, *d2, *d3, *d4,
            ...
            t, t1, t2, t3, t4, t5, dphi=0, dphi0, alpha, talpha,
...
Exit:
    ...
    if ( Parm.PrintFinal || Parm.PrintLevel )
    {
        ...
        const char mess5 [] = "   - the parameter epsilon in cg.parm "
                              "is too small" ;
        ...
        else if ( status < 0 )
        {
            if ( status == -1 )
            {
                printf ("cg_descent parameter file (cg_descent.parm) "
                        "not found\n") ;
            }
            else if ( status == -2 )
            {
                printf ("cg_descent.parm file is incomplete\n") ;
            }
            else if ( status == -3 )
            {
                printf ("the length of a comment statement in the "
                         "file g_descent.parm exceeds the maximum allowed "
                         "length %i\n", MAXLINE) ;
            }
        }
...
)
{
    ...
    ParmFile = fopen ("F:/workspace/VC_workspace/vcpkg_project/cg_descent/cg_descent_1.1/cg_descent.parm", "r") ;
    if ( ParmFile == NULL ) return (-1) ;

    info = fscanf (ParmFile, "%lg", &(Parm->delta)) ;
    if ( info != 1 )
    {
        printf ("delta parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    ...
    if ( info != 1 )
    {
        printf ("sigma parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("eps parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    ...
    if ( info != 1 )
    {
        printf ("gamma parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("rho parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("eta parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("psi0 parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("psi1 parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("psi2 parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("QuadCutOff parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("StopFact parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("AWolfeFac parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("restart_fac parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    ...
    if ( info != 1 )
    {
        printf ("maxit_fac parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    ...
    if ( info != 1 )
    {
        printf ("feps parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("Qdecay parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("nexpand parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("nsecant parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("PertRule parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("QuadStep parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("PrintLevel parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("PrintFinal parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("StopRule parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("AWolfe parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("Step parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
    ...
    if ( info != 1 )
    {
        printf ("debug parameter missing in cg_descent.parm\n") ;
        return (-2) ;
    }
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
...
{
    double zero, psi, dpsi, cg_dot (double *, double *, int) ;
    int n, nshrink, status, cg_Wolfe (double, double, double, cg_parameter *) ;
